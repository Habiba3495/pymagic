    ...
    const handleNextQuestion = async () => {
        if (currentQuestionIndex < questions.length - 1) {
            ...
            try {
                ...
                if (data.success) {
                    // Navigate with updated points and achievements
                    navigate("/quiz-complete", {
                        state: { 
                            quizData: data, 
                            total_user_points: data.total_user_points, // Updated total points
                            achievements: data.achievements || [] // New achievements (if any)
                        },
                    });
                } else {
                    console.error("Error submitting quiz:", data.message);
