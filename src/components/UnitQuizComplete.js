...
import bookIcon from "./images/Score icon.svg";
import pointsIcon from "./images/points.svg";
import "./QuizComplete.css"; // Assuming the CSS is shared or similar
import apiClient from '../services';
import { useTranslation } from "react-i18next";
import { useAuth } from '../context/AuthContext';
import trackEvent from '../utils/trackEvent';
...
const UnitQuizComplete = () => {
  ...
  const [newRewards, setNewRewards] = useState([]); // New state for rewards
  const [showRewardsPopup, setShowRewardsPopup] = useState(false); // State to control popup
  const { t } = useTranslation();
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchQuizData = async () => {
      ...
      // Extract new rewards from quizData
  ...
  // Function to close the rewards popup and proceed
